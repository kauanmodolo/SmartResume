...
const formConfig: FormConfigObject = {
  ...
  function getDefaultValues(formKey: ResumeArrayKeys) {
    const config = formConfig[formKey];
    const defaults: Record<string, any> = {};
    config.forEach(field => {
      defaults[field.key as string] = ""; // ou outro valor padrão
    });
    return defaults;
  }
  ...
  export const ManageMultipleItemDialog = ({ data, open, setOpen, initialData }: ManageMultipleItemDialogProps) => {
    ...
    return (
      <Dialog
        ...
        setOpen={setOpen}
        content={
          <form onSubmit={methods.handleSubmit(onSubmit)} className="flex flex-col mt-2">
